...
/**
 * POST /api/projects/[id]/inbox/conversations/[conversationId]/status
 * Updates status (e.g. mark resolved, reopen).
 */
  ...
  try {
    ...
    if (!['open', 'pending', 'resolved', 'closed'].includes(status)) {
      return NextResponse.json(
        { error: 'Invalid status. Must be open, pending, resolved, or closed.' },
        { status: 400 },
      );
    }

    const result = await inboxService.updateConversationStatus({
    ...
    console.error('[InboxAPI] Error updating status:', error);
    return NextResponse.json(
