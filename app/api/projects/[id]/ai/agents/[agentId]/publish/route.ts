...
/**
 * POST /api/projects/[id]/ai/agents/[agentId]/publish
 * Publishes the draft version into an immutable published version.
 */
export async function POST(
  ...
  try {
    ...
    const updated = await agentService.publishVersion({
    ...
    console.error('POST /api/projects/[id]/ai/agents/[agentId]/publish error:', err);
    return NextResponse.json({ status: 'error', error: err.message || 'Failed to publish agent version' }, { status: 400 });
  }
}
