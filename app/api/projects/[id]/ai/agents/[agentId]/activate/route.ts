...
/**
 * POST /api/projects/[id]/ai/agents/[agentId]/activate
 * Activates an agent.
 */
export async function POST(
  ...
  try {
    ...
    const updated = await agentService.activateAgent(workspace.id, projectId, agentId, user.id);
    ...
    console.error('POST /api/projects/[id]/ai/agents/[agentId]/activate error:', err);
    return NextResponse.json({ status: 'error', error: err.message || 'Failed to activate agent' }, { status: 400 });
  }
}
