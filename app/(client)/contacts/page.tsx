      ...
      try {
        ...
        if (res.ok) {
          ...
          if (json.status === 'ok' && Array.isArray(json.data) && json.data.length > 0) {
            router.replace(`/projects/${json.data[0].id}/contacts`);
            return;
          }
        }
      } catch (err) {
        console.error('Failed to resolve project for contacts:', err);
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="flex-1 overflow-y-auto p-6 sm:p-8 max-w-4xl mx-auto w-full">
      <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-2xs text-center">
        ...
        <p className="text-sm text-gray-500 max-w-md mx-auto mb-6">
          In Wazzi, contacts and customer interactions are scoped to projects. Create or select a project to manage your contacts and Customer 360.
        </p>
        <Link
          href="/projects"
