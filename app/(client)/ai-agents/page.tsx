...
import { Bot, FolderKanban, Loader2, Plus } from 'lucide-react';
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (json.status === 'ok' && Array.isArray(json.data) && json.data.length > 0) {
            router.replace(`/projects/${json.data[0].id}/ai/agents`);
            return;
          }
        }
      } catch (err) {
        console.error('Failed to resolve project for AI agents:', err);
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-12">
        ...
        <p className="text-sm font-semibold text-gray-700">Loading AI Agent Studio...</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 sm:p-8 max-w-4xl mx-auto w-full">
      <div className="bg-white rounded-3xl border border-gray-100 p-8 shadow-2xs text-center">
        ...
        <h2 className="text-xl font-extrabold text-gray-900 mb-2">Create a Project First</h2>
        <p className="text-sm text-gray-500 max-w-md mx-auto mb-6">
          In Wazzi, AI Agents are scoped to projects. Create or select a project to configure, test, and publish your autonomous AI agents.
        </p>
        <Link
          href="/projects"
